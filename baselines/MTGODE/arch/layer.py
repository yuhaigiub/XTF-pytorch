...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx):
        ...
        s1, t1 = (adj + torch.rand_like(adj) *
                  0.01).topk(self.k, 1)  # bug fixed
...
class CGPODEBlock(nn.Module):
    def __init__(self, cgpfunc, method, step_size, rtol, atol, adjoint, perturb, estimated_nfe):
        ...
        self.atol = atol
        self.rtol = rtol
        self.mlp = linear((estimated_nfe + 1) *
                          self.odefunc.c_in, self.odefunc.c_out)
    ...
    def __init__(self, cin, cout, alpha=2.0, method='rk4', time=1.0, step_size=1.0,
        ...
        if method == 'euler':
            ...
            self.estimated_nfe = round(
                self.integration_time / (step_size / 4.0))
        else:
