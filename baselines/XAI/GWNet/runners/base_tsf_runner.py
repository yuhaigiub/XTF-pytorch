...
from .runner_zoo.pertubate import FadeMovingAverage_TopK, perturbate_node
...
class BaseTimeSeriesForecastingRunner(BaseEpochRunner):
    ...
    def __init__(self, cfg: Dict):
        ...
        assert self.metrics_best in [
            'min', 'max'], f'Invalid best metric {self.metrics_best}.'
        ...
        # Eealuation settings
        self.if_evaluate_on_gpu = cfg.get(
            'EVAL', EasyDict()).get('USE_GPU', True)
        self.evaluation_horizons = [
            _ - 1 for _ in cfg.get('EVAL', EasyDict()).get('HORIZONS', [])]
        assert len(self.evaluation_horizons) == 0 or min(
            self.evaluation_horizons) >= 0, 'The horizon should start counting from 1.'
    ...
    def setup_graph(self, cfg: Dict, train: bool):
        ...
        dataloader = self.build_test_data_loader(
            cfg=cfg) if not train else self.build_train_data_loader(cfg=cfg)
    ...
    def curriculum_learning(self, epoch: int = None) -> int:
        ...
        if epoch < self.warm_up_epochs:
            ...
            progress = ((epoch - self.warm_up_epochs) //
                        self.cl_epochs + 1) * self.cl_step_size
    ...
    #         if 'null_val' not in metric_func.keywords and 'null_val' in covariate_names:
    ...
    def metric_forward(self, metric_func, args: Dict) -> torch.Tensor:
        ...
        Args:
            metric_func (function or functools.partial): Metric function.
            args (Dict or tuple): Arguments for metrics computation.
        ...
        if not isinstance(args, dict):
            raise TypeError(f'Expected args to be dict, but got {type(args)}')
    ...
    #                                                                     :cl_length, :, :]
    #         forward_return['target'] = forward_return['target'][:,
    #                                                             :cl_length, :, :]
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        model_return, blackbox_return = self.forward(
            data=data, epoch=epoch, iter_num=iter_num, train=True)

        if self.cl_param:
            cl_length = self.curriculum_learning(epoch=epoch)
            model_return['prediction'] = model_return['prediction'][:, :cl_length, :, :]
            blackbox_return['prediction'] = blackbox_return['prediction'][:, :cl_length, :, :]
            model_return['target'] = model_return['target'][:, :cl_length, :, :]
            blackbox_return['target'] = blackbox_return['target'][:, :cl_length, :, :]
            
        # Loss between model and blackbox predictions
        loss = self.metric_forward(self.loss, {'prediction': model_return['prediction'], 'target': blackbox_return['prediction']})

        self.update_epoch_meter('train/loss', loss.item())

        # Compute other metrics
        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, {'prediction': model_return['prediction'], 'target': blackbox_return['prediction']})
            self.update_epoch_meter(f'train/{metric_name}', metric_item.item())
    ...
    def test(self, train_epoch: Optional[int] = None, save_metrics: bool = False, save_results: bool = False) -> Dict:
        ...
        prediction_blackbox, target_blackbox, inputs_blackbox = [], [], []
        ...
        for data in tqdm(self.test_data_loader):
            ...
            if not self.if_evaluate_on_gpu:
                model_return['prediction'] = model_return['prediction'].detach().cpu()
                model_return['target'] = model_return['target'].detach().cpu()
                model_return['inputs'] = model_return['inputs'].detach().cpu()
                blackbox_return['prediction'] = blackbox_return['prediction'].detach().cpu()
                blackbox_return['target'] = blackbox_return['target'].detach().cpu()
                blackbox_return['inputs'] = blackbox_return['inputs'].detach().cpu()

            prediction.append(model_return['prediction'])
            target.append(model_return['target'])
            inputs.append(model_return['inputs'])
            prediction_blackbox.append(blackbox_return['prediction'])
            target_blackbox.append(blackbox_return['target'])
            inputs_blackbox.append(blackbox_return['inputs'])

        prediction = torch.cat(prediction, dim=0)
        target = torch.cat(target, dim=0)
        inputs = torch.cat(inputs, dim=0)
        prediction_blackbox = torch.cat(prediction_blackbox, dim=0)
        target_blackbox = torch.cat(target_blackbox, dim=0)
        inputs_blackbox = torch.cat(inputs_blackbox, dim=0)

        returns_all = {'prediction': prediction,
                       'target': target, 
                       'inputs': inputs,
                       'prediction_blackbox': prediction_blackbox,
                       'target_blackbox': target_blackbox,
                       'inputs_blackbox':inputs_blackbox}
        ...
        if save_results:
            ...
            np.savez(os.path.join(self.ckpt_save_dir,
                     'test_results.npz'), **test_results)
    ...
    def on_validating_end(self, train_epoch: Optional[int]):
        ...
        if train_epoch is not None:
            self.save_best_model(train_epoch, 'val/' +
                                 self.target_metrics, greater_best=greater_best)
