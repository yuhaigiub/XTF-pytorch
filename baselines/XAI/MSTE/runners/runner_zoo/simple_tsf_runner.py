...
from .pertubate import FadeMovingAverage_TopK, perturbate_node
...
class SimpleTimeSeriesForecastingRunner(BaseTimeSeriesForecastingRunner):
    ...
    def forward(self, data: Dict, epoch: int = None, iter_num: int = None, train: bool = True, **kwargs) -> Dict:
        ...
        top_k_spatial = 30
        top_k_temporal = 4     
        X_node = perturbate_node(saliency, adj_mx, top_k_spatial, True)
        timestep, _ = pertubate.apply(history_data.transpose(1,3), X_node.transpose(1,3), top_k_temporal, True)
        ...
        checkpoint = torch.load(r'D:\X_XFT_2\XTF-pytorch-v2\baselines\XAI\GWNet\blackbox.pt')
